...
c||"rotate"==c?a.transform+=d(c,h(b,2*e,c)):a[c]=d(c,h(b,2*e,c))}):r.exit.forEach(function(f){for(var c in f)"scale"==c||"translate3d"==c||"rotate"==c||"rotate3d"==c?a.transform+=d(c,h(f,2*(e-g[b].duration/2),c)):a[c]=d(c,h(f,2*(e-g[b].duration/2),c))});$(g[b].selector).css(a)},h=function(e,a,d){e=e[d];var f=g[b].duration;if("translate3d"==d||"rotate3d"==d){d={};for(axis in e.from)d[axis]=+(e.from[axis]+a/f*(e.to[axis]-e.from[axis])).toFixed(4);return d}return+(e.from+a/f*(e.to-e.from)).toFixed(4)};
return Object.freeze({init:k,addTransitions:function(b){for(var a in b){if(l[a]){console.log("Transition name already exists!!!");return}l[a]=b[a]}k()}})}();Space.init()}).call(this);